...
- `base.py` 只读加载原依赖库的45,785个类目，再合并项目内2,070条第五版官网覆盖数据；不修改site-packages。当前合并结果为45,840条有效基础类目。
- `engine.py` 先保留合并基础类目的精确匹配，再比较已审核规则的解析进度，最后返回明确的部分解析。
...
- 规则数据损坏时保留基础类目可用结果并提示；依赖或官网覆盖数据不可用时返回解析异常，但不重试国图。
...
| `tests/test_clc_parser.py` | 基础数据合并父链、复分样例、上下文隔离、停用号、未知部分与降级 |
| `tests/test_clc5_base_overlay.py` | 官网节点过滤、范围节点排除、不完整来源开关和方括号类目 |
...
- 官网原始树和JSONL：生成覆盖数据的本机来源文件；项目只保存筛选后的语义数据与来源摘要。

## 复分规则阶段与未完成项

### 组合分类路径解析

- 已完成全书本机候选扫描、原依赖与第五版官网可确认节点的合并基线、补充引擎和首批I/K/S及通用复分规则。
- `K23` 已使用第五版官网类名“古代史中期”；`K237.105` 的 `05` 仍属于待实现的K21/27仿K20规则，不由节点覆盖层猜测。
